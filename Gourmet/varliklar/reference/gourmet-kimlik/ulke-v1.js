/* =====================================================================
   ÜLKE KODU SEÇİCİSİ — `.fk-tel` / `.fk-cc*` bileşeninin davranışı
   Kaynak: Beyar'ın tasarım kaynağı, ortak kabuk JS'i (`kur()` sözleşmesi
   dâhil satır satır port)
   =====================================================================

   199 ülke: ISO2 · alan kodu · Türkçe ad. Bayrak GÖRSEL DEĞİL, ISO2'den
   üretilen bölgesel gösterge harfleridir — 199 bayrak dosyası taşınmaz,
   veri 3 KB'de kalır.

   🔴 NEDEN AYRI DOSYA: bileşen bugün iki kimlik ekranında birden basılıyor
   (`/giris` telefon segmentinde, `/kayit` telefon alanında). Veriyi
   `kimlik-v1.js`e koymak onu her iki ekranda da yükletirdi ki bu doğru,
   AMA veri ile davranışın ayrı durması ileride üçüncü bir yüzey (hesap
   ayarları telefon alanı) çıktığında ikinci bir kopya üretmeyi önler.

   ⚠ MARKA RENGİ TAŞIMAZ: bu dosyada renk YOK. Görünüm `kimlik-v1.css`
   BLOK 5'tedir ve orada ikame edilmiştir.
   ...
   KAP SÖZLEŞMESİ
     <div class="fk-tel" data-cc-kok>
       ...
       <div class="fk-cc-pop" role="dialog" hidden>
         <div class="fk-cc-search"><i class="fa-solid fa-magnifying-glass"></i><input type="text" /></div>
         <ul class="fk-cc-list" role="listbox" tabindex="-1"></ul>
         <p class="fk-cc-bos" hidden>Eşleşen ülke yok.</p>
       </div>
     </div>
     GOURMET_ULKE.kur();      // sayfadaki tüm [data-cc-kok] kaplarını sürer

   🔴 GİZLİ ALANIN ADI `phone_country`tir ve bu sunucu sözleşmesidir:
   `CreateNewUser` o adı `size:2` ile doğrular, `users.phone_country`
   kolonuna yazar. Kaynak `ulke` diyordu; ad DEĞİŞTİ çünkü bu projenin
   kolon adı o.

   Klavye sözleşmesi: ↑ ↓ gezinir · Enter seçer · Esc kapatır ve odak
   düğmeye döner · yazmak süzer · dışarı tıklayınca kapanır.

   ⚠ JS'SİZ HÂL: düğme hiçbir şey yapmaz, açılır hiç açılmaz — ama gizli
   alan `value="TR"` ile doğar ve telefon alanı yazılabilir kalır. Yani
   form JS olmadan da GÖNDERİLEBİLİR; yalnız ülke Türkiye'de sabitlenir.
   ===================================================================== */
window.GOURMET_ULKE = (function () {
  'use strict';
  ...
  function kurBir(kok) {
    if (kok.__ccKuruldu) return;          /* iki kez surulmesin */
    ...
    /* acilis gorunumu — gizli input'taki ISO neyse o basilir */
